...
import { usePickOrderId, useUpdateOrderItemsStatus } from '@/hooks/pickStationHook';
import { getUserRoleHelper } from '@/lib/authHelper';
import { motion } from 'framer-motion';
import { Box, Package, CreditCard, Info, X, Check } from 'lucide-react';
...
export const OrderDetails = ({ order, onClose }: OrderDetailsProps) => {
  const role = getUserRoleHelper();
  const { data, isLoading, refetch } = usePickOrderId(order.id);
  const updateItemsMutate = useUpdateOrderItemsStatus();
  ...
  const handleConfirmPickup = (items: any[]) => {
    ...
    if (readyItems.length > 0) {
      ...
      setVerificationCode(readyItems[0].randomCode);
  ...
  const readyForPickupItems = orderData.items.filter((item: any) => 
    item.itemStatus === OrderStatus.DELIVERED
  );

  const showPickupButton = 
    role === 'pickupstation' && 
    readyForPickupItems.length > 0;
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {showPickupButton && (
                  <div className="mt-4">
                    <button
                      onClick={() => handleConfirmPickup(orderData.items)}
                    ...
                    >
                      <Check size={18} /> Confirm Customer Pickup
                    </button>
                  </div>
                )}
              </div>
