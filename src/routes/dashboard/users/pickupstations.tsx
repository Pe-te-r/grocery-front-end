...
export const Route = createFileRoute('/dashboard/users/pickupstations')({
...
function SuperAdminsComponent() {
  const { data, isLoading, isError } = useQuery({
    queryKey: ['pick-stations'],
  ...
  const columns: ColumnDef<SuperAdmin>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <button
          onClick={() => {
            console.log('Viewing details for super admin ID:', row.original.id)
  ...
  if (isError) {
    return (
      <div className="flex justify-center items-center h-64">
        <p className="text-red-500">Failed to load pick stations users</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-sm">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Pickup stations owners</h1>
      </div>
      <UserTable
        data={superAdmins || []}
        isLoading={isLoading}
