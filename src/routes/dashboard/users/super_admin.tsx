...
export const Route = createFileRoute('/dashboard/users/super_admin')({
...
function SuperAdminsComponent() {
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => getUsersFn({ superadmin: true }),
  })
  ...
  if (isError) {
    return (
      <div className="flex justify-center items-center h-64">
        <p className="text-red-500">Failed to load super admins</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-sm">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Super Admins</h1>
      </div>
      <UserTable
        data={superAdmins || []}
        isLoading={isLoading}
